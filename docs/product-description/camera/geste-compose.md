...
Deux doigts posés sur la scène commandent deux choses à la fois, dans un même mouvement continu : le glissement des deux doigts oriente la caméra (comme l'[orbite libre](orbite-libre.md), mais avec une précision réduite à 72 %), et le pincement règle le zoom. Les deux se combinent librement — on peut s'approcher d'une planète en tournant autour, sans lever les doigts. Il n'y a pas de geste de torsion : l'horizon reste toujours à plat. Le geste composé s'arrête avec les doigts : contrairement à l'orbite libre, il n'a jamais d'élan. S'il commence pendant un glissement à un doigt, il *prend la main* : le glissement est annulé net, sans élan. Une exception : quand une sonde est sélectionnée, le pincement ne règle pas la distance de caméra mais l'ampleur du cadrage de sa trajectoire, de 0,3× à 3×.

## Le cas simple

L'utilisateur pose deux doigts sur la scène et les écarte : la caméra s'approche de sa cible. Il glisse les deux doigts ensemble vers la gauche : la scène tourne devant lui, un peu moins vite que d'un seul doigt, ce qui donne un contrôle plus fin. Les faire tourner l'un autour de l'autre, en revanche, ne fait rien : l'horizon reste à plat.

Tout se fait en même temps, sans mode ni ordre imposé : le mouvement des doigts est décomposé en continu en écartement et déplacement, et chaque composante agit sur son axe. Il n'y a pas de « verrouillage » sur la composante dominante : même un pincement franc emporte un peu d'orientation si les doigts dérivent, et c'est voulu — le geste est une manipulation à main levée, pas deux outils séparés.

> Note technique : les deux composantes sont deux reconnaisseurs distincts autorisés à se reconnaître simultanément, chacun livrant ses deltas image par image. C'est ce qui rend la combinaison continue possible — et c'est aussi pourquoi le geste composé ne doit jamais avoir d'élan : les vitesses de plusieurs reconnaisseurs ne doivent jamais s'additionner.
...
stateDiagram-v2
    ...
    poses --> compose : une composante dépasse son seuil (déplacement ou écartement)
    orbiteLibre --> compose : un deuxième doigt se pose (prendre la main, sans élan)
    compose --> compose : orientation et zoom se mettent à jour en continu
...
Les deux composantes s'engagent chacune à son propre seuil système : il suffit que les doigts se déplacent assez (orientation), ou s'écartent ou se resserrent assez (zoom). En pratique un mouvement naturel les engage presque ensemble. À l'engagement de chaque composante :
...
- l'orientation abandonne toute visée de caméra programmée (recadrage vers une sonde, un satellite, un pas de tir) — le doigt gagne toujours ;
- le zoom capture la distance (ou l'ampleur) du moment : tout le pincement sera mesuré par rapport à cet instant.

> Note technique : le pincement, lui, n'abandonne pas la visée d'orientation — ce n'est pas un geste d'orientation. Un pincement pur, sans glissement, laisse la caméra finir son recadrage angulaire pendant que la distance obéit aux doigts. Il reprend en revanche la main sur toute animation de distance en cours.

### Pendant le geste

Chaque image, les deux axes suivent les doigts :
...
Tout s'arrête net, à l'endroit exact où le geste l'a laissé : ni élan d'orientation, ni zoom résiduel. Le geste composé n'a jamais d'élan, quelle que soit la vivacité du lâcher — c'est voulu, pour que les vitesses de plusieurs gestes ne puissent jamais s'additionner (voir [Gestes et caméra](../foundations/gestes-et-camera.md#larbitrage--qui-prend-la-main-sur-qui)).
...
Rien n'est « acté » : angle, distance et ampleur ne sont pas persistés, le geste suivant repart de là.
...
| Cartouche déplié | Le geste fonctionne sur la partie visible de la scène ; le cadrage reste décalé vers la moitié haute, zoom compris. | Aucun effet. |
...
| Le système annule le toucher | Rien n'avait commencé ; rien à annuler. | Le geste s'arrête là où il en est — sans élan, comme toute fin de geste composé. Angle et distance conservés. |
...
**Caméra et cadrage.** Le geste possède les trois nombres du rig à la fois : azimut, élévation (à 72 %) et distance (ou ampleur). L'orientation abandonne la visée programmée à l'engagement ; le pincement reprend la main sur l'animation de distance. Constantes et bornes dans [Gestes et caméra](../foundations/gestes-et-camera.md).
...
**Sélection.** Le geste ne change jamais la sélection ; il en dépend deux fois : pour savoir autour de quoi orbiter, et pour savoir si le pincement règle la distance ou l'ampleur. Changer de sonde remet l'ampleur à 1.

**Réseau et replis.** Aucune interaction.

**Échelles compressées.** Le zoom traverse l'espace de scène compressé : un même écartement de doigts couvre beaucoup plus de kilomètres réels loin du Soleil que près de la Terre. Les angles d'orientation, eux, ne sont pas compressés.

**Localisation et langue.** Aucune interaction : le geste n'affiche aucun texte.

**Accessibilité.** Le geste composé n'a pas d'équivalent accessibilité (pas d'action VoiceOver pour zoomer). La scène ne propose aucun contrôle alternatif de caméra.

## Cas limites

- **Bouger un seul des deux doigts, l'autre posé immobile** : c'est toujours le geste composé qui répond — le glissement à un doigt ne s'engage jamais tant que deux doigts sont posés. Le déplacement est mesuré au point médian des deux doigts, donc l'orientation avance environ moitié moins vite, et le mouvement produit au passage un peu de zoom.
- **Pincer pendant un recadrage programmé** : un pincement pur laisse la visée d'orientation se finir — la caméra continue de tourner vers son but pendant que la distance suit les doigts. Dès que les doigts glissent, la visée est abandonnée.
...
- **Atteindre la butée d'élévation à deux doigts** : comme à un doigt, la vue ne monte plus mais l'azimut et le zoom continuent de répondre.
...
- **Pincer pendant qu'un lancement est sélectionné** : sans effet. Tant que le lancement est sélectionné (pendant la séquence de tir et après), l'app impose la distance de caméra à chaque image — le pincement la règle, mais elle est aussitôt reprise. L'orientation, elle, répond normalement : on peut regarder le tir sous l'angle qu'on veut, pas à la distance qu'on veut. Signalé comme défaut possible.
...
- Les seuils d'engagement des deux composantes (déplacement, écartement) sont ceux des reconnaisseurs système, non redéfinis par l'app ; valeurs non mesurées.
